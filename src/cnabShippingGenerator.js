const { CNAB_SHIPPING_BRADESCO_ESPECIFICATION } = require("./constants");
  ...
  generate() {
    ...
    for (const [key, value] of Object.entries(this.#header)) {
      const fieldEspecification =
        CNAB_SHIPPING_BRADESCO_ESPECIFICATION.header[key];
    ...
    const transactions = this.#transactions.map((transaction) => {
      ...
      for (const [key, value] of Object.entries(transaction)) {
        const fieldEspecification =
          CNAB_SHIPPING_BRADESCO_ESPECIFICATION.transactions[key];
        const startIndex = fieldEspecification.startIndex;
        const endIndex = fieldEspecification.endIndex;
    ...
    for (const [key, value] of Object.entries(this.#trailler)) {
      const fieldEspecification =
        CNAB_SHIPPING_BRADESCO_ESPECIFICATION.trailler[key];
      const startIndex = fieldEspecification.startIndex;
      const endIndex = fieldEspecification.endIndex;
